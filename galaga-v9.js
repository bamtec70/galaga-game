...
(() => {
   ...
   * 5) if last ship: respawn (life lost). If dual/triple: remaining ships keep playing.
   */
  function startCaptureSequence(boss) {
    ...
    if (hadMulti) {
      // One ship pulled; you keep flying the rest — no full freeze, no life cost
      state = "play";
      invuln = 1200; // brief i-frames after losing a wingman
      sfx("capture");
      flashMsg("FIGHTER CAPTURED!", 1600);
    } else {
      // Last ship: full cinematic, control lost until respawn
      player.alive = false;
  ...
  function updateCaptureSequence(dt) {
    ...
    if (seq.phase === "lock") {
      ...
      // Only for last-ship capture. Partial capture never reaches here with life loss.
      if (seq.partial) {
        captureSeq = null;
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V9 — PRESS SPACE OR TAP");
})();
