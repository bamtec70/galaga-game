...
(() => {
  ...
  function divePathFor(e, opts) {
    ...
    // Approach beam altitude (or dive mid-screen)
    const midY = opts.beamDive ? VH * 0.42 : VH * 0.55;
    for (let i = 1; i <= 48; i++) {
      const t = i / 48;
      pts.push({
    ...
    if (opts.beamDive) {
      // Mark beam start index — boss freezes here for tractor
      e.beamAtIndex = pts.length - 1;
      // After beam, return home (filled later if capture) or exit
      const home = slotPos(e.slot, 0);
      for (let i = 1; i <= 40; i++) {
        const t = i / 40;
        pts.push({
          x: lerp(targetX, home.x, t),
          y: lerp(midY, home.y, t * t), // ease up into formation
        });
      }
    } else {
      // exit sweep
  ...
  function beginTractorBeam(e) {
    if (e.state === "beam" || e.hasCaptive || capturedHeld) return;
    if (!player || !player.alive) return;
    e.state = "beam";
    e.beamT = 0;
    e.beamDuration = 2800;
    e.angle = Math.PI / 2; // face down
    // Lock horizontal near player for a fair capture window
    e.beamX = clamp(player.x + rnd(-10, 10), 40, VW - 40);
    e.x = lerp(e.x, e.beamX, 0.5);
  ...
  function performCapture(e) {
    if (!player || !player.alive || e.hasCaptive || capturedHeld) return;
    if (invuln > 0) return;

    sfx("capture");
    burst(player.x, player.y, C.cyan, 18);
    ...
    e.state = "return";
    e.pathPos = 0;
    const home = slotPos(e.slot, formSway);
    e.returnPath = [];
    for (let i = 0; i <= 40; i++) {
      const t = i / 40;
      e.returnPath.push({
        x: lerp(e.x, home.x, t),
        y: lerp(e.y, home.y, t * t),
      });
    }
    beams = beams.filter((b) => b.enemy !== e);

    if (dual) {
      // Dual → single; one fighter is the captive (classic dual risk)
      dual = false;
      player.w = 18;
      invuln = 2000;
    ...
    // Single fighter: capture costs a life
    lives--;
    hud();
    flashMsg("FIGHTER CAPTURED!", 2000);
    if (lives <= 0) {
  ...
  function startDive(e) {
    ...
    // Boss: often attempt tractor beam when no captive yet
    const tryBeam =
      e.type === "boss" &&
      !capturedHeld &&
      !e.hasCaptive &&
  ...
  function updateEnemies(dt) {
    ...
    for (const e of enemies) {
      ...
      if (e.state === "enter") {
        ...
        // Hold still and project tractor cone — do NOT advance dive path
        e.beamT = (e.beamT || 0) + dt;
        e.angle = Math.PI / 2;
        if (player && player.alive) {
          e.x = lerp(e.x, player.x, 0.03); // slight track
        }
        // Cone geometry (matches drawBeams)
        const coneLen = 170;
        const coneHalf = 42; // wide enough for larger sprites
        if (player && player.alive && invuln <= 0) {
          const dy = player.y - e.y;
          const dx = player.x - e.x;
          if (dy > 12 && dy < coneLen) {
            const halfAtY = 6 + (dy / coneLen) * coneHalf;
            if (Math.abs(dx) < halfAtY) {
              // Pull player into beam (arcade "suck")
              player.x = lerp(player.x, e.x, 0.08);
              // Capture once held near center for a moment
              e.captureHold = (e.captureHold || 0) + dt;
              if (e.captureHold > 280 || Math.abs(dx) < 14) {
                performCapture(e);
              }
            } else {
              e.captureHold = Math.max(0, (e.captureHold || 0) - dt);
            }
          }
        }
        // Sync beam visual
        for (const b of beams) {
          if (b.enemy === e) {
            b.x = e.x;
            b.y = e.y;
            b.life = e.beamDuration - e.beamT;
          }
        }
        if (e.beamT >= e.beamDuration && e.state === "beam") {
          // Missed — resume return/exit path
          e.state = "dive";
          e.beamed = true;
          e.willBeam = false;
          beams = beams.filter((b) => b.enemy !== e);
          // Jump pathPos to post-beam segment if present
          if (e.beamAtIndex >= 0) {
            e.pathPos = e.beamAtIndex + 0.01;
          }
        }
        ...
        // Start tractor when boss reaches beam altitude
        if (
          e.type === "boss" &&
          e.willBeam &&
          !e.beamed &&
        ...
        const done = followPath(e, e.divePath, dt, diveSpeed);

        // Shoot while diving (not during beam)
        e.shootT -= dt;
        ...
        // Ram collision (not while beaming — beam handles capture)
        if (player && player.alive && invuln <= 0 && e.state === "dive") {
        ...
        if (done) {
          ...
          e.pathPos = 0;
          const fp = slotPos(e.slot, formSway);
          // Re-enter from top if exited bottom/sides
  ...
  function drawBeams() {
    // Cone must match capture geometry (coneLen 170, coneHalf 42)
    const coneLen = 170;
    const coneHalf = 42;
    for (const beam of beams) {
      if (!beam.enemy || beam.enemy.state !== "beam") continue;
      const x = beam.enemy.x;
      const y = beam.enemy.y;
      const pulse = 0.55 + 0.45 * Math.sin(performance.now() / 85);
      for (let i = 0; i < 12; i++) {
        const t0 = i / 12;
        const t1 = (i + 1) / 12;
        const w0 = 5 + t0 * coneHalf * pulse;
        const w1 = 5 + t1 * coneHalf * pulse;
        const y0 = y + 12 + t0 * coneLen;
        const y1 = y + 12 + t1 * coneLen;
        ctx.globalAlpha = 0.12 + 0.14 * ((i % 2) + pulse * 0.5);
        ctx.fillStyle = i % 2 ? "#40ffff" : "#e0ffff";
        ctx.beginPath();
        ctx.moveTo(x - w0, y0);
        ctx.lineTo(x + w0, y0);
        ctx.lineTo(x + w1, y1);
      ...
      ctx.globalAlpha = 1;
      fillRect(x - 5, y + 8, 10, 5, "#a0ffff");
      fillRect(x - 2, y + 6, 4, 3, "#ffffff");
    }
  }
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V4 — PRESS SPACE OR TAP");
})();
