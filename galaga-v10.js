...
(() => {
   ...
   * 5) if last ship: game over after cinematic. If dual/triple: remaining keep playing.
   */
  function startCaptureSequence(boss) {
    ...
    // Always capture exactly ONE fighter
    const hadMulti = fighterCount > 1;
    const takeSide = fighterCount >= 2 ? (chance(0.5) ? -1 : 1) : 0; // which wing is taken
    ...
    captureSeq = {
      ...
      red: 0,
      beamPulse: 0,
      // partial: remaining fighter(s) stay under player control
      partial: hadMulti,
      fromDual: hadMulti,
      lastFighter: !hadMulti,
    };

    if (hadMulti) {
      // One ship pulled; you keep flying the rest — no full freeze
      state = "play";
      invuln = 1200;
      sfx("capture");
      flashMsg("FIGHTER CAPTURED!", 1600);
    } else {
      // Last fighter: full cinematic, then GAME OVER (no respawn)
      player.alive = false;
      player.capturing = true;
      state = "capture";
      bullets = [];
      sfx("capture");
      flashMsg("LAST FIGHTER CAPTURED!", 2000);
    }
  }
  ...
  function updateCaptureSequence(dt) {
    ...
    if (seq.phase === "lock") {
      ...
      // Last fighter was captured → game over (no life respawn)
      if (seq.partial) {
        captureSeq = null;
      ...
      if (seq.t >= 1200) {
        ...
        hud();
        state = "over";
        showOV("GAME OVER", "FIGHTER CAPTURED", "PRESS SPACE OR TAP");
      }
    }
  }
  ...
  function drawPlayer() {
    ...
    const x = player.x;
    const y = player.y;
    // During rescue approach, don't show the joining ship until dock
    let n = fighterCount;
    if (rescueSeq && !rescueSeq.docked) {
      // fighterCount already includes only current ships; rescue is separate sprite
      n = fighterCount;
    }
    if (n <= 1) {
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V10 — PRESS SPACE OR TAP");
})();
