/**
 * GALAGA — Namco / Midway 1981 style recreation (Build V12)
 * Arcade-faithful Namco WSG-style SFX + redrawn pixel sprites.
...
(() => {
  ...
  // ── Audio (Namco WSG-style synthesis — same fidelity bar as Dig Dug) ─────
  let AC = null;
  let master = null;
  let namcoWave = null;
  let noiseBuf = null;
  ...
  function unlockAudio() {
    try {
      if (!AC) {
        ...
        // Soft-edged pulse approximating Namco custom wavetable chips
        const n = 32;
  ...
  function tone(freq, dur, type = "square", vol = 0.04, when = 0, slideTo) {
    ...
    try {
      ...
      g.gain.exponentialRampToValueAtTime(vol * 0.45, t0 + Math.min(0.05, dur * 0.35));
  ...
  function noise(dur, vol = 0.05, when = 0, ff = 1200, q = 0.8) {
  ...
  /** Pulsing tractor-beam drone (arcade-character rising rings). */
  function beamPulse(seconds) {
    const steps = Math.max(4, Math.floor(seconds * 8));
    for (let i = 0; i < steps; i++) {
      const t = i * 0.12;
      const f = 140 + (i % 4) * 38;
      tone(f, 0.14, "namco", 0.028, t);
      tone(f * 2.02, 0.12, "triangle", 0.012, t + 0.02);
      noise(0.08, 0.012, t, 900 + i * 40, 0.6);
    }
  }
  ...
  function sfx(name) {
    ...
    if (muted || !AC) return;
    if (name === "fire") {
      // Sharp high "pew" — Galaga fighter shot character
      tone(1480, 0.045, "square", 0.032, 0, 620);
      tone(2200, 0.03, "square", 0.016, 0.01, 900);
      noise(0.025, 0.012, 0, 3200, 1.2);
    } else if (name === "hit") {
      // Boss armor clang (first hit)
      tone(310, 0.07, "square", 0.04);
      tone(520, 0.05, "namco", 0.028, 0.02);
      noise(0.06, 0.028, 0, 700, 0.7);
    } else if (name === "kill") {
      // Enemy explosion — noise bloom + descending chirps
      noise(0.16, 0.07, 0, 1400, 0.55);
      noise(0.22, 0.045, 0.04, 500, 0.4);
      tone(520, 0.1, "square", 0.03, 0, 120);
      tone(280, 0.14, "namco", 0.028, 0.04, 70);
      tone(180, 0.12, "triangle", 0.02, 0.08, 50);
    } else if (name === "die") {
      // Player ship destroy
      noise(0.45, 0.09, 0, 900, 0.5);
      noise(0.35, 0.06, 0.08, 320, 0.35);
      tone(480, 0.35, "sawtooth", 0.045, 0, 55);
      tone(240, 0.4, "namco", 0.035, 0.05, 40);
      tone(120, 0.3, "triangle", 0.025, 0.12, 35);
    } else if (name === "dive") {
      // Iconic Galaga dive warble (descending square steps)
      const dive = [740, 660, 590, 520, 470, 420, 380, 340, 300, 270];
      dive.forEach((f, i) => {
        tone(f, 0.055, "square", 0.028, i * 0.045);
        tone(f * 0.5, 0.055, "namco", 0.014, i * 0.045);
      });
    } else if (name === "beam") {
      beamPulse(0.9);
    } else if (name === "capture") {
      // Eerie capture descent
      const cap = [420, 380, 340, 300, 260, 220, 190, 160, 130];
      cap.forEach((f, i) => {
        tone(f, 0.09, "namco", 0.038, i * 0.07);
        tone(f * 1.5, 0.08, "triangle", 0.016, i * 0.07);
      });
      noise(0.35, 0.035, 0.15, 450, 0.5);
    } else if (name === "rescue") {
      seq(
        [
          [523, 0.07],
  ...
  function startCaptureSequence(boss) {
    ...
    // Always capture exactly ONE on-screen fighter
    ...
    captureSeq = {
      ...
      red: 0,
      beamPulse: 0,
      partial: hadMulti, // remaining on-screen ships keep playing
      fromDual: hadMulti,
    };
    ...
    if (hadMulti) {
      ...
      // Solo fighter captured — full cinematic; stock life resolved after haul
      player.alive = false;
  ...
  function updateCaptureSequence(dt) {
    ...
    if (seq.phase === "lock") {
      ...
      if (boss && boss.state === "form") {
        beams = beams.filter((b) => b.enemy !== boss);
        if (seq.partial) {
          // Remaining fighter(s) already in play — end sequence
          captureSeq = null;
        } else {
          seq.phase = "respawn";
          seq.t = 0;
      ...
      // Solo ship was captured: spend one stock fighter (start with 3).
      // Respawn if stock remains; only the LAST stock fighter → game over.
      if (seq.partial) {
        captureSeq = null;
      ...
      if (seq.t >= 1200) {
        ...
        beams = [];
        if (lives > 0) {
          // Still have stock fighters — return to play
          fighterCount = 1;
          spawnPlayer();
          invuln = 2500;
          state = "play";
          ...
          // That was the last fighter — game over
          fighterCount = 0;
          state = "over";
  ...
  function update(dt) {
    ...
    if (state === "capture") {
      // Full cinematic when the LAST ship is captured
    ...
    updateBeams(dt);
    updateParticles(dt);
    // Partial capture (one of dual/triple) runs alongside normal play
    if (captureSeq && captureSeq.partial) {
      updateCaptureSequence(dt);
      for (const b of beams) {
        if (captureSeq && b.enemy === captureSeq.boss) b.life = 9999;
      }
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V12 — PRESS SPACE OR TAP");
})();
