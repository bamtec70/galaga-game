...
(() => {
  ...
  function beginTractorBeam(e) {
    ...
    e.captureHold = 0;
    e.angle = Math.PI / 2;
    // Sit above the player so the cone clearly covers them
    const py = player.y;
    e.y = Math.min(e.y, py - 190);
    e.y = Math.max(FORM_OY + 40, e.y);
    ...
    sfx("beam");
    flashMsg("TRACTOR BEAM!", 900);
  }

  function performCapture(e) {
    if (!player || !player.alive || e.hasCaptive || capturedHeld) return;
    // Invuln only blocks for a brief post-spawn window; allow capture if beam has been on ≥0.4s
    if (invuln > 0 && (e.beamT || 0) < 400) return;

    sfx("capture");
    burst(player.x, player.y, C.cyan, 20);
    burst(player.x, player.y, C.white, 10);
    e.hasCaptive = true;
    capturedHeld = true;
    e.willBeam = false;
    e.beamed = true;
    ...
    if (dual) {
      ...
      player.w = 18;
      invuln = 2000;
      flashMsg("FIGHTER CAPTURED!", 2200);
      return;
    }
  ...
  function updateEnemies(dt) {
    ...
    for (const e of enemies) {
      ...
      if (e.state === "enter") {
        ...
        // Hold and project tractor cone — do NOT advance dive path
        e.beamT = (e.beamT || 0) + dt;
        e.angle = Math.PI / 2;
        // Track player so staying under the beam is fair
        if (player && player.alive) {
          e.x = lerp(e.x, player.x, 0.06);
        }
        if (player && player.alive) {
          if (playerInBeamCone(e)) {
            // Strong pull toward center of beam
            player.x = lerp(player.x, e.x, 0.14);
            e.captureHold = (e.captureHold || 0) + dt;
            // Capture quickly once inside the cone
            if (e.captureHold > 180) {
              performCapture(e);
            }
          } else {
            e.captureHold = Math.max(0, (e.captureHold || 0) - dt * 0.5);
          }
        }
        ...
        if (e.state === "beam" && e.beamT >= e.beamDuration) {
          e.state = "dive";
          e.beamed = true;
          e.willBeam = false;
          beams = beams.filter((b) => b.enemy !== e);
  ...
  function drawBeams() {
    for (const beam of beams) {
      ...
      const x = e.x;
      const y = e.y;
      const py = player && player.alive ? player.y : VH - 48;
      const coneLen = Math.max(220, py - y + 50);
      const coneHalf = 55;
      const pulse = 0.55 + 0.45 * Math.sin(performance.now() / 85);
      for (let i = 0; i < 14; i++) {
        const t0 = i / 14;
        const t1 = (i + 1) / 14;
        const w0 = 8 + t0 * coneHalf * pulse;
        const w1 = 8 + t1 * coneHalf * pulse;
        const y0 = y + 14 + t0 * coneLen;
        const y1 = y + 14 + t1 * coneLen;
        ctx.globalAlpha = 0.14 + 0.14 * ((i % 2) + pulse * 0.5);
        ctx.fillStyle = i % 2 ? "#40ffff" : "#e8ffff";
        ctx.beginPath();
        ctx.moveTo(x - w0, y0);
        ctx.lineTo(x + w0, y0);
        ctx.lineTo(x + w1, y1);
      ...
      ctx.globalAlpha = 1;
      fillRect(x - 6, y + 8, 12, 6, "#a0ffff");
      fillRect(x - 2, y + 6, 4, 4, "#ffffff");
    }
  }
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V5 — PRESS SPACE OR TAP");
})();
