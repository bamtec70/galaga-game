/**
 * GALAGA — Namco / Midway 1981 style recreation (Build V14)
 * V14: major fidelity overhaul — Dig Dug/Joust-scale sprites, capture FSM
 * hardening, hostile captive, classic challenge cadence, retuned SFX.
...
(() => {
  ...
  function startBeamHum(seconds) {
    ...
    try {
      ...
      o2.type = "sine";
      o1.frequency.setValueAtTime(132, t0);
      o2.frequency.setValueAtTime(264, t0);
      o1.frequency.linearRampToValueAtTime(155, t0 + 0.4);
      o1.frequency.linearRampToValueAtTime(125, t0 + 0.85);
      o1.frequency.linearRampToValueAtTime(148, t0 + 1.2);
      o2.frequency.linearRampToValueAtTime(310, t0 + 0.6);
      g.gain.setValueAtTime(0.0001, t0);
      g.gain.exponentialRampToValueAtTime(0.038, t0 + 0.06);
  ...
  function sfx(name) {
    ...
    if (muted || !AC) return;
    if (name === "fire") {
      // Classic short bright zap
      tone(1480, 0.045, "square", 0.04, 0, 620);
      tone(2100, 0.03, "namco2", 0.016, 0.006, 900);
      noise(0.018, 0.012, 0, 4200, 1.5, 2000);
    } else if (name === "hit") {
      ...
      // Descending siren — Galaga dive character
      const dive = [820, 740, 670, 600, 540, 485, 430, 385, 345, 310, 280, 250, 225];
      dive.forEach((f, i) => {
        const t = i * 0.038;
        tone(f, 0.055, "square", 0.032, t);
        tone(f * 0.5, 0.055, "namco", 0.018, t);
        if (i % 3 === 0) tone(f * 1.5, 0.03, "triangle", 0.01, t + 0.008);
      });
    } else if (name === "beam") {
      startBeamHum(1.15);
      beamPulse(0.55);
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V14 — PRESS SPACE OR TAP");
})();
