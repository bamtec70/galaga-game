/**
 * GALAGA — Namco / Midway 1981 style recreation (Build V18)
 * V18: same V17 arcade PCB fidelity + WSG audio, with softer/rounder
 * sprite silhouettes (fighter, bee, butterfly, Boss green→blue, captured
 * fighter, explosions) — less blocky corners, more arcade-organic shapes.
 * Audio/FSM unchanged. Keeps capture/dual-fighter FSM.
...
(() => {
  ...
  const SPR_PX = 2.05;
  const SPR_PX_BOSS = 2.2;
  const SPR_PX_SHIP = 2.1;
  ...
  const WSG_WAVES_4BIT = [
    // 0 sine-ish (smooth lead)
    [7,9,10,11,12,13,13,14,14,14,13,13,12,11,10,9,7,5,4,3,2,1,1,0,0,0,1,1,2,3,4,5],
    // 1 gated-buzzy (tractor / dive wail) — classic Galaga grit
    [7,9,10,11,7,13,13,7,14,7,13,13,7,11,10,9,7,5,7,3,7,1,7,0,7,0,7,1,7,3,7,5],
    ...
    // 4 rich double-sine / brass
    [7,10,12,13,14,13,12,10,7,4,2,1,0,1,2,4,7,11,13,14,13,11,7,3,1,0,1,3,7,14,7,0],
    // 5 metallic sparse (hits / sizzle)
    [7,14,12,9,12,14,10,7,12,15,13,8,10,11,7,2,8,13,9,4,5,7,2,0,3,8,5,1,3,6,3,1],
    // 6 warm saw / organ body (fanfare)
    [7,8,10,12,14,13,12,12,11,10,8,7,5,6,7,8,8,9,10,11,9,8,6,5,4,4,3,2,4,6,8,9],
    // 7 hollow organ (challenge / climb)
  ...
  function startBeamHum(seconds) {
    ...
    beamLoopUntil = tEnd;
    // Cabinet tractor: wave1 gated drone + wave4 brass sub — heavy LFO, not POKEY.
    let phase = 0;
    vset(0, 108, 14, 1, true);
    vsched(1, 0, 216, 6, 4, true);
    beamTimer = setInterval(() => {
      ...
      phase += 0.52;
      const wob = Math.sin(phase) * 48 + Math.sin(phase * 0.37) * 22 + Math.sin(phase * 2.6) * 10;
      const volWob = 11 + Math.floor(3 + 3 * Math.sin(phase * 2.1));
      vset(0, 100 + wob, Math.min(15, volWob), 1, false);
      vset(1, 200 + wob * 1.7, 5 + (volWob >> 2), 4, false);
    }, 30);
  ...
  function sfx(name) {
    ...
    if (name === "fire") {
      // Arcade Galaga zip: brief LFSR grit + high square→saw chirp (WSG, not TIA).
      noise(0.014, 0.038, 0, 8200, 2.6, 2800);
      noise(0.028, 0.018, 0.006, 2200, 1.1, 800);
      vset(2, 2920, 15, 2, true);
      vsched(2, 0.006, 2280, 14, 2, false);
      vsched(2, 0.014, 1680, 12, 2, false);
      vsched(2, 0.026, 1120, 10, 6, false);
      vsched(2, 0.038, 740, 7, 6, false);
      vsched(2, 0.05, 420, 3, 4, false);
      vsched(2, 0.062, 0, 0, 2, false);
    } else if (name === "hit") {
      ...
      // Boom: multi-crest LFSR + metallic wave5 sizzle + low thud (arcade, not POKEY).
      noise(0.1, 0.11, 0, 2400, 0.55, 480);
      noise(0.2, 0.085, 0.04, 950, 0.42, 200);
      noise(0.38, 0.055, 0.1, 340, 0.32, 90);
      vset(1, 300, 14, 6, true);
      vsched(1, 0.03, 170, 13, 5, false);
      vsched(1, 0.08, 100, 10, 5, false);
      vsched(1, 0.16, 62, 6, 0, false);
      vsched(1, 0.28, 0, 0, 0, false);
      vset(2, 210, 12, 2, true);
      vsched(2, 0.05, 110, 9, 5, false);
      vsched(2, 0.14, 58, 5, 0, false);
      vsched(2, 0.26, 0, 0, 0, false);
    } else if (name === "die") {
      ...
      // Cabinet dive cry: gated-buzzy wave1 + organ body wave6, wider warble.
      const steps = 30;
      for (let i = 0; i < steps; i++) {
        const t = i * 0.036;
        const base = 1040 * Math.pow(0.902, i);
        const warble = Math.sin(i * 2.35) * (55 + i * 2.4) + Math.sin(i * 0.62) * 22;
        const vol = Math.max(4, 14 - Math.floor(i / 3.2));
        vsched(1, t, Math.max(68, base + warble), vol, 1, i === 0);
        vsched(2, t, Math.max(52, (base + warble) * 0.5), Math.max(2, vol - 3), 6, i === 0);
      }
      vsched(1, steps * 0.036 + 0.03, 0, 0, 1, false);
      vsched(2, steps * 0.036 + 0.03, 0, 0, 6, false);
    } else if (name === "beam") {
      startBeamHum(1.35);
      for (let i = 0; i < 10; i++) {
        const t = i * 0.09;
  ...
  // V18: softer rounded silhouette (curved wing tips, oval body)
  const SPR_FIGHTER = [
    "........W........",
    ".......WWW.......",
    "......WwWwW......",
  ...
  // Bee / Zako — 4 wing frames, blue thorax, yellow abdomen, white eyes
  // V18: oval body + rounded wing lobes
  const SPR_BEE = [
    [
      "...bb.....bb...",
      "..bBBb...bBBb..",
  ...
  // Butterfly / Goei — magenta/red wings, yellow body, 4 flap frames
  // V18: lobed rounded wings, softer outline
  const SPR_BUTTER = [
    [
      ".MM.........MM.",
      "MMmRR.....RRmMM",
  ...
  // V18: rounded bird body, curved wing tips
  const SPR_BOSS = [
    [
      ".L.GG.....GG.L.",
      "LGGGGg...gGGGGL",
  ...
  // Boss damaged — classic blue after first hit (same rounded silhouette)
  const SPR_BOSS_HURT = [
    [
      ".C.BB.....BB.C.",
      "CBBBBb...bBBBBC",
