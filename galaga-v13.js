/**
 * GALAGA — Namco / Midway 1981 style recreation (Build V13)
 * V13: capture sequence freeze fix + richer Namco-style Web Audio SFX.
...
(() => {
  ...
  // ── Audio (richer Namco WSG-style synthesis — no ROM samples) ─────────────
  let AC = null;
  let master = null;
  let namcoWave = null;
  let namcoWave2 = null;
  ...
  let muted = false;
  let beamLoopUntil = 0;
  let beamHumNodes = null; // live tractor-beam drone
  ...
  function unlockAudio() {
    try {
      if (!AC) {
        ...
        for (let i = 1; i < n; i++) {
          imag[i] = (1 / i) * Math.sin(i * Math.PI * 0.32);
        }
        const real2 = new Float32Array(n);
        const imag2 = new Float32Array(n);
        for (let i = 1; i < n; i++) {
          imag2[i] = (1 / i) * Math.sin(i * Math.PI * 0.18);
        }
        try {
          namcoWave = AC.createPeriodicWave(real, imag);
          namcoWave2 = AC.createPeriodicWave(real2, imag2);
        } catch (_) {
          namcoWave = null;
          namcoWave2 = null;
        }
  ...
  function tone(freq, dur, type = "square", vol = 0.04, when = 0, slideTo) {
    if (muted || !AC || !dest()) return;
    try {
      const t0 = AC.currentTime + when;
      const o = AC.createOscillator();
      const g = AC.createGain();
      if (type === "namco" && namcoWave) o.setPeriodicWave(namcoWave);
      else if (type === "namco2" && namcoWave2) o.setPeriodicWave(namcoWave2);
      else if (type === "namco" || type === "namco2") o.type = "triangle";
      else o.type = type;
      o.frequency.setValueAtTime(Math.max(20, freq), t0);
      if (slideTo != null) {
        o.frequency.exponentialRampToValueAtTime(Math.max(20, slideTo), t0 + Math.max(0.01, dur));
      }
      g.gain.setValueAtTime(0.0001, t0);
      g.gain.exponentialRampToValueAtTime(vol, t0 + 0.003);
      g.gain.exponentialRampToValueAtTime(vol * 0.55, t0 + Math.min(0.04, dur * 0.3));
  ...
  function beamPulse(seconds) {
    const steps = Math.max(6, Math.floor(seconds * 10));
    for (let i = 0; i < steps; i++) {
      const t = i * 0.1;
      const f = 120 + (i % 5) * 42;
      tone(f, 0.12, "namco", 0.03, t);
      tone(f * 2.01, 0.1, "triangle", 0.012, t + 0.015);
      tone(f * 3.02, 0.08, "namco2", 0.008, t + 0.03);
      noise(0.07, 0.014, t, 850 + i * 55, 0.7);
    }
  }

  function stopBeamHum() {
    if (!beamHumNodes || !AC) return;
    try {
      const t0 = AC.currentTime;
      beamHumNodes.g.gain.cancelScheduledValues(t0);
      beamHumNodes.g.gain.setValueAtTime(Math.max(0.0001, beamHumNodes.g.gain.value), t0);
      beamHumNodes.g.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.08);
      beamHumNodes.o1.stop(t0 + 0.1);
      beamHumNodes.o2.stop(t0 + 0.1);
    } catch (_) {}
    beamHumNodes = null;
    beamLoopUntil = 0;
  }
  ...
  function startBeamHum(seconds) {
    ...
    try {
      ...
      const g = AC.createGain();
      if (namcoWave) o1.setPeriodicWave(namcoWave);
      else o1.type = "triangle";
      o2.type = "sine";
      o1.frequency.setValueAtTime(148, t0);
      o2.frequency.setValueAtTime(296, t0);
      o1.frequency.linearRampToValueAtTime(168, t0 + 0.35);
      o1.frequency.linearRampToValueAtTime(140, t0 + 0.7);
      o1.frequency.linearRampToValueAtTime(160, t0 + 1.1);
      g.gain.setValueAtTime(0.0001, t0);
      g.gain.exponentialRampToValueAtTime(0.034, t0 + 0.06);
      g.gain.setValueAtTime(0.034, t0 + Math.max(0.1, seconds - 0.12));
      ...
      o1.start(t0);
      o2.start(t0);
      o1.stop(t0 + seconds + 0.05);
      o2.stop(t0 + seconds + 0.05);
      beamHumNodes = { o1, o2, g };
      beamLoopUntil = performance.now() + seconds * 1000;
    } catch (_) {
      beamPulse(seconds);
    }
  }
  ...
  function sfx(name) {
    ...
    if (muted || !AC) return;
    if (name === "fire") {
      tone(1650, 0.038, "square", 0.036, 0, 720);
      tone(2400, 0.028, "namco2", 0.018, 0.008, 1100);
      tone(980, 0.02, "triangle", 0.01, 0.002, 500);
      noise(0.022, 0.014, 0, 3800, 1.4, 1800);
    } else if (name === "hit") {
      tone(290, 0.08, "square", 0.045);
      tone(480, 0.06, "namco", 0.03, 0.015);
      tone(720, 0.04, "namco2", 0.018, 0.03);
      noise(0.07, 0.032, 0, 650, 0.75, 320);
    } else if (name === "kill") {
      noise(0.14, 0.078, 0, 1600, 0.5, 420);
      noise(0.26, 0.05, 0.03, 700, 0.4, 180);
      tone(560, 0.09, "square", 0.034, 0, 130);
      tone(300, 0.14, "namco", 0.03, 0.035, 75);
      tone(190, 0.16, "triangle", 0.022, 0.07, 45);
      tone(110, 0.12, "namco2", 0.016, 0.1, 40);
    } else if (name === "die") {
      noise(0.5, 0.1, 0, 1100, 0.45, 200);
      noise(0.4, 0.07, 0.06, 400, 0.35, 90);
      tone(520, 0.38, "sawtooth", 0.05, 0, 60);
      tone(260, 0.42, "namco", 0.038, 0.04, 42);
      tone(140, 0.36, "triangle", 0.028, 0.1, 32);
      tone(90, 0.28, "namco2", 0.02, 0.16, 28);
    } else if (name === "dive") {
      const dive = [780, 700, 630, 560, 500, 450, 400, 360, 320, 285, 255, 230];
      dive.forEach((f, i) => {
        const t = i * 0.042;
        tone(f, 0.06, "square", 0.03, t);
        tone(f * 0.5, 0.06, "namco", 0.016, t);
        if (i % 2 === 0) tone(f * 1.49, 0.035, "namco2", 0.01, t + 0.01);
      });
    } else if (name === "beam") {
      startBeamHum(1.15);
      beamPulse(0.55);
      ...
      stopBeamHum();
      startBeamHum(1.6);
      const cap = [460, 420, 380, 340, 300, 265, 230, 200, 170, 145, 125, 105];
      cap.forEach((f, i) => {
        const t = i * 0.075;
        tone(f, 0.1, "namco", 0.04, t);
        tone(f * 1.5, 0.09, "triangle", 0.018, t);
        tone(f * 0.5, 0.1, "namco2", 0.014, t + 0.02);
      });
      noise(0.55, 0.04, 0.12, 520, 0.55, 160);
      noise(0.4, 0.025, 0.35, 280, 0.4, 90);
    } else if (name === "rescue") {
      seq(
        [
          [523, 0.065],
      ...
      tone(1320, 0.08, "namco", 0.032, 0.04);
      noise(0.08, 0.03, 0.02, 1400, 0.8, 600);
    }
  }
  ...
  function syncFighters() {
    dual = fighterCount >= 2;
    if (player) {
      player.w = fighterCount === 1 ? 18 : fighterCount === 2 ? 36 : 50;
    }
  }
  ...
  function startCaptureSequence(boss) {
    ...
    const capX = player.x + takeSide * (fighterCount >= 3 ? 20 : 14);
    const capY = player.y;
  ...
  function updateCaptureSequence(dt) {
    ...
    if (boss && boss.state === "dead" && seq.phase !== "resolve" && seq.phase !== "respawn") {
      ...
      if (almostDocked && seq.partial && player && player.alive && !rescueSeq) {
        startRescueSequence(seq.fx || boss.x, seq.fy || boss.y + 18);
  ...
  function fire() {
    ...
    const y = player.y - 12;
    if (n === 1) {
      bullets.push({ x: player.x, y, vy: -230, friendly: true });
    } else if (n === 2) {
      bullets.push({ x: player.x - 10, y, vy: -230, friendly: true });
      bullets.push({ x: player.x + 10, y, vy: -230, friendly: true });
    } else {
      bullets.push({ x: player.x - 16, y, vy: -230, friendly: true });
      bullets.push({ x: player.x, y, vy: -230, friendly: true });
      bullets.push({ x: player.x + 16, y, vy: -230, friendly: true });
    }
    sfx("fire");
  ...
  function killEnemy(e, diving) {
    ...
    // Captive rescue: destroy boss with captive → spin/dock sequence
    if (hadCaptive) {
      e.hasCaptive = false;
      capturedHeld = false;
      // Last alien on screen? Always allow retrieve (stage would wipe rescue otherwise)
      const othersLeft = enemies.some((o) => o !== e && o.state !== "dead");
      const canRescue =
        hadCaptive &&
        (diveRescue || !othersLeft); // diving boss OR final alien with captive
  ...
  function updateRescueSequence(dt) {
    ...
    if (seq.phase === "spin") {
      ...
      const dur = 450;
      const spacing = fighterCount >= 2 ? 20 : 14;
      if (player && player.alive) {
  ...
  function updatePlayer(dt) {
    ...
    const margin = fighterCount >= 3 ? 28 : fighterCount === 2 ? 22 : 14;
    ...
    for (const e of enemies) {
      ...
      if (e.state === "enter") {
        ...
        if (player && player.alive && invuln <= 0) {
          const hitR =
            (fighterCount >= 3 ? 20 : fighterCount === 2 ? 16 : 11) +
            (e.type === "boss" ? 6 : 0);
  ...
  function updateBullets(dt) {
    ...
    if (player && player.alive && invuln <= 0) {
      for (const b of eBullets) {
        const hitW = fighterCount >= 3 ? 20 : fighterCount === 2 ? 16 : 10;
  ...
  function drawPlayer() {
    ...
    if (n <= 1) {
      blit(x, y, SPR_FIGHTER, 1.15);
    } else if (n === 2) {
      blit(x - 14, y, SPR_FIGHTER, 1.1);
      blit(x + 14, y, SPR_FIGHTER, 1.1);
    } else {
      blit(x - 20, y, SPR_FIGHTER, 1.05);
      blit(x, y, SPR_FIGHTER, 1.1);
      blit(x + 20, y, SPR_FIGHTER, 1.05);
    }
  }
  ...
  function drawEnemy(e) {
    ...
    if (e.type === "bee") {
      blit(0, 0, frame ? SPR_BEE_B : SPR_BEE_A, 1.1);
    } else if (e.type === "butterfly") {
      blit(0, 0, frame ? SPR_BUTTER_B : SPR_BUTTER_A, 1.15);
    } else if (e.type === "boss") {
      const hurt = e.hp < 2;
  ...
  function drawBossSprite(ox, oy, hurt, frame) {
    ...
    const rows = hurt
      ...
      : frame
        ? SPR_BOSS_B
        : SPR_BOSS_A;
    blit(ox, oy, rows, 1.25);
  }

  function drawBullets() {
    for (const b of bullets) {
      // Arcade twin-pixel fighter missile (white tip / yellow body)
      fillRect(b.x - 1, b.y - 8, 2, 11, "#ffff48");
      fillRect(b.x - 1, b.y - 8, 2, 3, "#ffffff");
      fillRect(b.x - 2, b.y - 2, 4, 2, "#e0c020");
    }
    for (const b of eBullets) {
      // Galaga enemy bomb — pink/magenta diamond
      fillRect(b.x - 1, b.y - 4, 2, 8, "#ff70b8");
      fillRect(b.x - 2, b.y - 2, 4, 4, "#ff3090");
  ...
  hud();
  if ($high) $high.textContent = pad(high);
  showOV("GALAGA", "INSERT COIN", "BUILD V13 — PRESS SPACE OR TAP");
})();
